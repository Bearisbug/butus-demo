...
| 演练实例:全新建仓 10 分钟上线 + 故障/恢复/维护全链路走通 | 待验证 |
| UI 浏览器实测:四要素(分组状态/30 天色条/时间线/暗色)截图存档 | 待验证 |
| Lighthouse ≥95、LCP 达标(§3 口径) | 过(2026-08-01 本地实测:mobile 100/LCP 0.9s,desktop 100/LCP 0.2s;证据 docs/test-runs/lighthouse-*.json) |
| 仓库无明文密钥;文档含 Secrets 用法与私有仓库限制说明 | 待验证 |
| 模板出厂态:`data/` 已清空(仅 .gitkeep),不携带本地演示数据 | 待验证 |
| README + 使用文档与实现一致 | 待验证 |
